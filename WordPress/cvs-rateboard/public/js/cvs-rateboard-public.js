	...
	$.fn.cvsRateboard = function(options){
		...
		return this.each(function(i, v) {
			...
			const data_freq					= $(this).data('freq'), 
				  ...
				  data_rb_col				= $(this).data('rb-col'), 
				  data_rb_bck_col			= $(this).data('rb-bck-col'), 
				  data_rb_border_col		= $(this).data('rb-border-col'), 
				  data_rb_table_th_back		= $(this).data('rb-table-th-back'), 
				  data_rb_table_even_bck	= $(this).data('rb-table-even-bck'), 
				  data_rb_table_odd_bck		= $(this).data('rb-table-odd-bck'), 
				  data_rb_logo_url			= $(this).data('rb-logo-url'), 
				  data_rb_show_slider		= $(this).data('rb-show-slider'),
				  data_rb_slide_urls		= $(this).data('rb-slide-urls'), 
				  data_rb_has_slides		= $(this).data('rb-has-slides');
			...
			options.style['logo'] 					= data_rb_logo_url,
			options.style['color'] 					= data_rb_col,
			options.style['backgroundColor'] 		= data_rb_bck_col,
			options.style['borderColor'] 			= data_rb_border_col,
			options.style['headerBackgroundColor'] 	= data_rb_table_th_back, 
			options.style['evenBackgroundColor'] 	= data_rb_table_even_bck,
			options.style['oddBackgroundColor'] 	= data_rb_table_odd_bck,
			options.slider['display'] 				= ( data_rb_show_slider )
														? true
														: false;

  			const slides_Arr = [];

  			$.each( data_rb_slide_urls, function(i, v) {
  				slides_Arr.push( v );
  			})

  			if ( slides_Arr.length > 0 )
				options.slider['slider_images'] 	= slides_Arr;
			...
			// Set up constants
			const rateboard 		= options.rateboard,
				  ...
				  url_pattern 		= new RegExp(
			   	  ...
			   	  hasLogo 		= ( 
			   	  					('' !== options.style['logo'] ) && 
			   	  					(
			   	  						( url_pattern.test( options.style['logo'] ) ) ||
			...
			if ( 
				...
				$.each( images, function(i, v) {
					...
					if ( 
						...
						console.log('Some of your slider image urls is/are incorrect.');
					}
			...
			function buildRatesTable( data ) { 
				...
				// We need to manually add the ISFLAGGED column
				thArr.push({
					'key': "isflagged",
					'value': 9,
					'label': "Is Flagged",
							...
							if ( false !== options.updateFrequency ) {
								...
								reparseXml = setInterval(function() { 
									...
									//if ( i <= 10 ) {
								    	...
								    	// The looped Ajax request - here we use the default
